...
function expandableCard(id, label, value, foot, info, variant, tableHtml, hint){
  const v = variant || 'blue';
  const icon = KPI_ICONS[v] || KPI_ICONS.blue;
  return `<div class="kpi-card k-${v} audit-card" data-audit="${id}">
    <div class="kpi-top">
...
function renderResumen(c, themeChanged){
  ...
  if(!themeChanged){
    const pg = r.precios_globales, pr = r.presencia;
    const gapMin = (pg.propio_min != null && pg.comp_min) ? diferencial(pg.propio_min, pg.comp_min) : null;
    const gapMax = (pg.propio_max != null && pg.comp_max) ? diferencial(pg.propio_max, pg.comp_max) : null;
    c.innerHTML = `
    <div class="kpi-grid">
      ...
      ${kpiCard('Presencia Competencia (share)', pr.pct_competencia + '%',
        `${fmtNum(pr.pdvs_con_competencia)} de ${fmtNum(pr.pdvs_total)} PDVs auditados${pr.lidera==='competencia' ? ' · Lidera' : ''}`,
        `De los PDVs con al menos un precio relevado en Envasados dentro del rango elegido (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}), qué porcentaje tiene presencia de alguna marca de competencia relevada.`,
        'accent')}
      ${kpiCard('Precio mínimo — Minerva vs. Competencia', `${fmtGs(pg.propio_min)} <span style="font-size:12px; color:var(--text3); font-weight:500;">vs</span> ${fmtGs(pg.comp_min)}`,
        gapMin == null ? null : (gapMin < 0 ? `Minerva ${Math.abs(gapMin)}% más barata en su precio mínimo` : `Minerva ${gapMin}% más cara en su precio mínimo`),
        `Precio mínimo relevado de Minerva vs. precio mínimo relevado de competencia, agregando todos los cortes juntos dentro del rango elegido (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}). No es por corte — para eso está la comparación de abajo.`,
        'blue', true)}
      ${kpiCard('Precio máximo — Minerva vs. Competencia', `${fmtGs(pg.propio_max)} <span style="font-size:12px; color:var(--text3); font-weight:500;">vs</span> ${fmtGs(pg.comp_max)}`,
        gapMax == null ? null : (gapMax < 0 ? `Minerva ${Math.abs(gapMax)}% más barata en su precio máximo` : `Minerva ${gapMax}% más cara en su precio máximo`),
        `Precio máximo relevado de Minerva vs. precio máximo relevado de competencia, agregando todos los cortes juntos dentro del rango elegido (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}). No es por corte — para eso está la comparación de abajo.`,
        'blue', true)}
      ${(() => {
        const cp = r.cortes_premium, pm = cp.propio_max;
        const tabla = `<div class="table-scroll"><table>
          <thead><tr><th>Corte</th><th>Marca</th><th class="num">Precio máximo</th></tr></thead>
          <tbody>${cp.propio_por_familia.map(x => x.corte
            ? `<tr><td>${x.familia}${x.corte===pm?.corte && x.marca===pm?.marca ? ' <span class="tag tag-alta" style="margin-left:4px;">Máximo de los 3</span>' : ''}</td><td>${x.marca}</td><td class="num">${fmtGs(x.precio_max)}</td></tr>`
            : `<tr><td>${x.familia}</td><td colspan="2" style="color:var(--text3);">Sin relevamientos en el rango elegido</td></tr>`).join('')}
          </tbody></table></div>`;
        return expandableCard('premium-minerva', 'Precio máximo — Cortes premium Minerva', pm ? fmtGs(pm.precio_max) : '—',
          pm ? `${pm.corte} - ${pm.marca}` : 'Sin relevamientos de Tapa/Vacío/Costilla en el rango elegido',
          'Precio máximo relevado entre Tapa (incl. Tapa De Cuadril), Vacío y Costilla (todas sus variantes), cualquier marca propia, dentro del rango de fechas elegido arriba. Tocá la tarjeta para ver el máximo de cada uno de los 3 por separado.',
          'teal', tabla, 'Ver el máximo de cada corte');
      })()}
      ${(() => {
        const cp = r.cortes_premium;
        const tabla = cp.competencia_top3.length ? `<div class="table-scroll"><table>
          <thead><tr><th>Corte</th><th>Marca</th><th class="num">Precio máximo</th></tr></thead>
          <tbody>${cp.competencia_top3.map(x => `<tr><td>${x.corte}</td><td>${x.marca}</td><td class="num">${fmtGs(x.precio_max)}</td></tr>`).join('')}</tbody>
        </table></div>` : `<div style="font-size:12px; color:var(--text3); padding:4px 0;">Sin datos de competencia en el rango elegido.</div>`;
        const top = cp.competencia_top3[0];
        return expandableCard('premium-competencia', 'Precio máximo — Top 3 cortes Competencia', top ? fmtGs(top.precio_max) : '—',
          top ? `${top.corte} - ${top.marca}` : 'Sin relevamientos de competencia en el rango elegido',
          'Los 3 cortes más caros relevados en competencia dentro del rango de fechas elegido arriba, sin restringir a ningún corte en particular. Tocá la tarjeta para ver el ranking completo.',
          'accent', tabla, 'Ver el ranking completo');
      })()}
    </div>
