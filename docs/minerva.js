...
function renderResumen(c, themeChanged){
  ...
  if(!themeChanged){
    ...
    <div class="card">
      ...
      ${(() => {
        // Familia de corte = primera palabra significativa, no el nombre
        // completo -- así "Costilla Especial" (propio) sí cuenta como con
        // competencia si existe cualquier "Costilla ..." de competencia,
        // sin exigir que el nombre completo coincida letra por letra.
        // Regla de negocio confirmada por Di: cualquier corte de Minerva
        // compite con alguno de competencia -- no existe "sin competencia"
        // de verdad, así que además de los precios relevados en Envasados
        // se cruza contra la hoja de marcas de competencia (FRIGO/COMPETENCIA),
        // que es la fuente autorizada de qué compite con qué.
        const familiaDe = corte => normSearch(corte).split(' ')[0];
        const familiasConCompetenciaRelevada = new Set(DATA.precios.competencia_detalle.map(x => familiaDe(x.corte)));
        const familiasDeclaradas = new Set(DATA.competencia.marcas_compe_por_corte.map(x => familiaDe(x.corte)));
        const sinCompetencia = DATA.precios.propio_detalle
          .filter(x => !familiasConCompetenciaRelevada.has(familiaDe(x.corte)) && !familiasDeclaradas.has(familiaDe(x.corte)))
    ...
    function competenciaDelCorte(){
      ...
      return [...crossCompOpts.keys()].filter(k => crossCompOpts.get(k).corte === corte).sort();
    }
