...
function renderResumen(c, themeChanged){
  ...
  if(!themeChanged){
    ...
    // --- Cruce directo: corte propio vs. corte de competencia, a nivel CORTE - MARCA ---
    // (nombres de corte pueden no coincidir entre lados, y un mismo corte
    // puede tener varias marcas de competencia -- se identifica cada una
    // por separado en vez de mezclarlas bajo el mismo nombre de corte.)
    ...
    const cortesCompList = [...crossCompOpts.keys()].sort();
    if (!crossMinervaCorte || !crossMinervaOpts.has(crossMinervaCorte)) {
      crossMinervaCorte = cortesMinervaList[0] || null;
    }
    if (!crossCompCorte || !crossCompOpts.has(crossCompCorte)) {
      const corteElegido = crossMinervaCorte ? crossMinervaOpts.get(crossMinervaCorte).corte : null;
      crossCompCorte = (corteElegido && cortesCompList.find(k => crossCompOpts.get(k).corte === corteElegido)) || cortesCompList[0] || null;
    }
  ...
  function renderCrossResult(){
    ...
    const cRow = crossCompOpts.get(crossCompCorte);
    if (!mRow || !cRow) {
    ...
    box.innerHTML = `
      <div class="table-scroll"><table>
        <thead><tr><th>Comparación</th><th class="num"><span class="tag tag-propio">Minerva</span> ${mRow.corte} - ${mRow.marca}</th><th class="num"><span class="tag tag-comp">Competencia</span> ${cRow.corte} - ${cRow.marca}</th><th class="num">Diferencia</th></tr></thead>
        <tbody>
          <tr><td>Precio mínimo relevado</td><td class="num">${fmtGs(mRow.precio_min)}</td><td class="num">${fmtGs(cRow.precio_min)}</td>
            <td class="num"><span class="tag ${gapMin<0?'tag-propio':'tag-comp'}">${gapMin>0?'+':''}${gapMin}%</span></td></tr>
          <tr><td>Precio máximo relevado</td><td class="num">${fmtGs(mRow.precio_max)}</td><td class="num">${fmtGs(cRow.precio_max)}</td>
            <td class="num"><span class="tag ${gapMax<0?'tag-propio':'tag-comp'}">${gapMax>0?'+':''}${gapMax}%</span></td></tr>
          <tr><td>Cantidad de relevamientos</td><td class="num">${mRow.n}</td><td class="num">${cRow.n}</td><td class="num">—</td></tr>
        </tbody>
      </table></div>
  ...
  function drawCrossChart(mRow, cRow){
    ...
    charts['crossCompare'] = new Chart(canvas, {
      type: 'bar',
      data: {
        labels: [`Competencia · ${cRow.corte} - ${cRow.marca}`, `Minerva · ${mRow.corte} - ${mRow.marca}`],
        datasets: [{ data: [[cRow.precio_min,cRow.precio_max],[mRow.precio_min,mRow.precio_max]], backgroundColor: [accent, teal], borderRadius: 6, maxBarThickness: 30, minBarLength: 4 }]
      },
      options: {
