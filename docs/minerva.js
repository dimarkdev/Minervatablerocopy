...
function renderResumen(c, themeChanged){
  ...
  if(!themeChanged){
    c.innerHTML = `
    <div class="kpi-grid">
      ${kpiCard('Cobertura vigente', r.vigentes_30d + ' / ' + r.total_locales,
        `${Math.round(100*r.vigentes_30d/r.total_locales)}% del universo con precio fresco en el rango elegido`,
        `Locales con al menos un precio propio (Pul/Estancia 92) cargado dentro del rango de fechas elegido arriba (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}), sobre el total de locales monitoreados. Cambiá el rango de fechas para recalcular.`)}
      ${kpiCard('Más barata en el precio mínimo', r.precio_por_corte.filter(x=>x.diferencial_min_pct<0).length+'/'+r.precio_por_corte.length,
        ...
        `Compara el precio mínimo relevado de Minerva vs. el precio mínimo relevado de la competencia dentro del rango elegido (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}), corte por corte. Cuenta en cuántos cortes el precio mínimo de Minerva es más barato.`, 'accent')}
      ${kpiCard('Más barata en el precio máximo', r.precio_por_corte.filter(x=>x.diferencial_max_pct<0).length+'/'+r.precio_por_corte.length,
        ...
        `Compara el precio máximo relevado de Minerva vs. el precio máximo relevado de la competencia dentro del rango elegido (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}), corte por corte. Cuenta en cuántos cortes el precio máximo de Minerva es más barato.`, 'accent')}
      ${kpiCard('Locales monitoreados', fmtNum(r.total_locales),
        ...
        'Cantidad total de puntos de venta distintos con al menos un registro de precio (propio o competencia) en toda la base histórica. No cambia con el rango de fechas de arriba — es el tamaño del universo monitoreado, no una métrica de actividad reciente.')}
    </div>
