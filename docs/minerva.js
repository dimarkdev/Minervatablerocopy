...
function renderVigencia(c, themeChanged){
  ...
  <div class="kpi-grid">
    ...
    ${expandableCard('vig-vencimientos', 'Vencimientos', fmtNum(v.total),
      `${yaVencido && yaVencido.n>0 ? `${yaVencido.n} ya vencidos` : ''}${yaVencido && yaVencido.n>0 && en7dias && en7dias.n>0 ? ' · ' : ''}${en7dias && en7dias.n>0 ? `${en7dias.n} vencen en 7 días` : ''}${(!yaVencido || yaVencido.n===0) && (!en7dias || en7dias.n===0) ? 'Nada urgente por ahora' : ''}`,
      'Cantidad de registros de productos con fecha de vencimiento relevada en góndola, clasificados por urgencia (ya vencido / vence en 7 días / vence en 30 días / más adelante).',
    ...
    ${kpiCard('Corte recurrente en alertas', cr ? cr.corte : '—',
      ...
      'El corte que más se repite entre los registros "ya vencido" o "vence en 7 días" — no es el que más unidades tiene, es el que más seguido vuelve a aparecer en alerta. Sirve para identificar un problema recurrente de rotación o reposición en ese corte puntual, más que una unidad aislada.',
      'amber')}
  </div>
  ...
  <div class="card">
    ...
    ${v.urgentes.length ? `<div class="table-scroll"><table>
      ...
      <tbody>${v.urgentes.map(x=>`<tr${x.dias<=-10?' style="background:var(--accent-soft);"':''}><td>${x.pdv}</td><td>${x.corte}</td><td>${x.marca}</td><td class="num">${fmtNum(x.cantidad)}</td><td class="num">${x.fecha_vence.toLocaleDateString('es-PY')}</td><td class="num">${x.dias<0?`Vencido hace ${Math.abs(x.dias)}d`:`${x.dias}d`}</td></tr>`).join('')}</tbody>
...
async function bootstrapData(){
  ...
  try {
    ...
    if (_primeraCarga) {
      // Primera carga: siempre arranca en los últimos 3 meses, sin
      // importar qué valores hardcodeados traigan los inputs de fecha.
      windowStart = new Date(now.getTime() - VENTANA_DIAS * 86400000);
      windowEnd = now;
      _primeraCarga = false;
    } else {
