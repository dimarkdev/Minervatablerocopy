  ...
  function renderCrossResult(){
    ...
    box.innerHTML = `
      <div class="cross-result">
        <div class="cross-box cross-box-propio"><span class="tag tag-propio">Minerva</span><div class="cross-corte">${mRow.corte}</div>
          <div class="cross-range">${fmtGs(mRow.propio_min)} – ${fmtGs(mRow.propio_max)}</div>
          <div class="cross-n">${mRow.n_propio} relevamiento(s)</div></div>
        <div class="cross-gap">
          <div class="cross-gap-pct" style="color:${gapMin<0?'var(--teal)':'var(--accent)'}">${gapMin>0?'+':''}${gapMin}%</div>
          <div class="cross-gap-label">${gapMin<0?'Minerva más barata (precio mínimo)':'Minerva más cara (precio mínimo)'}</div>
        </div>
        <div class="cross-box cross-box-comp"><span class="tag tag-comp">Competencia</span><div class="cross-corte">${cRow.corte}</div>
          <div class="cross-range">${fmtGs(cRow.comp_min)} – ${fmtGs(cRow.comp_max)}</div>
          <div class="cross-n">${cRow.n_competencia} relevamiento(s)</div></div>
      </div>
      <div class="alert-note" style="margin-top:10px;">Precio máximo: ${fmtGs(mRow.propio_max)} (Minerva) vs. ${fmtGs(cRow.comp_max)} (competencia) — ${gapMax>0?'+':''}${gapMax}%</div>`;
  ...
  if(!themeChanged){
    ...
    <div class="card">
      ...
      <div class="note-box top">
        ...
        <span>Este relevamiento es del mostrador de carnicería de los supermercados: no sabemos qué frigorífico/marca vende cada precio, solo si ese día el súper tenía un precio de oferta cargado distinto del regular. Sirve para ver si hay un patrón semanal de promoción (ej. "jueves de descuento") y usarlo para decidir <b>cuándo</b> promocionar los cortes de carne de Minerva en esa carnicería, no solo los envasados.</span>
      </div>
      ${cn.mejor_dia_oferta ? `
    ...
    <div class="card">
      ...
      <div class="cross-grid" style="grid-template-columns:repeat(auto-fill, minmax(190px,1fr));">
        ${cn.patrones_oferta.top_cortes_por_cadena.map(x => `
        <div class="cross-box"><div class="cross-corte" style="margin-bottom:8px;">${x.cadena}</div>
          ${x.top.length ? x.top.map((c,i)=>`<div class="alert-note" style="font-style:normal; margin-top:4px;">${i+1}. ${c.corte} — ${c.n}</div>`).join('') : `<div class="alert-note">Sin ofertas detectadas</div>`}
        </div>`).join('')}
      </div>` : `
