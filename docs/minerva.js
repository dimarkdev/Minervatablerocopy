const DATA = JSON.parse(document.getElementById('minerva-data').textContent);
...
function renderAlertPanel(){
  ...
  const cards = [
    alertCard('prioridad', al.prioridad, `<table><thead><tr><th>Local</th><th>Categoría</th><th class="num">Días sin relevar</th></tr></thead><tbody>
      ${al.prioridad.detalle.map(x=>`<tr><td>${x.local}</td><td>${x.categoria}</td><td class="num">${x.dias_sin_relevar}</td></tr>`).join('')}
      </tbody></table><div class="alert-note">${al.prioridad.nota}</div>`),
    alertCard('cobertura', al.cobertura, `<table><thead><tr><th>Local</th><th class="num">Días sin relevar</th></tr></thead><tbody>
      ${al.cobertura.detalle.map(x=>`<tr><td>${x.local}</td><td class="num">${x.dias_sin_relevar}</td></tr>`).join('')}
      </tbody></table><div class="alert-note">Mostrando los 15 más atrasados de ${al.cobertura.total} en total.</div>`),
    alertCard('pedidos', al.pedidos, `<table><thead><tr><th>Repositor</th><th class="num">Días sin pedido</th></tr></thead><tbody>
    ...
    alertCard('precio', al.precio, al.precio.total===0
      ...
      : `<table><thead><tr><th>Corte</th><th class="num">Diferencial</th></tr></thead><tbody>
        ${al.precio.detalle.map(x=>`<tr><td>${x.corte}</td><td class="num">+${x.diferencial_pct}%</td></tr>`).join('')}
        </tbody></table>`),
    alertCard('disponibilidad', al.disponibilidad, `<table><thead><tr><th>Corte</th><th class="num">Jun</th><th class="num">Jul (parcial)</th><th class="num">Variación</th></tr></thead><tbody>
      ...
      </tbody></table><div class="alert-note">${al.disponibilidad.nota}</div>`),
  ];
...
function bindAlertPanel(){
  document.querySelectorAll('#alertPanel .alert-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      document.querySelectorAll('#alertPanel .alert-detail').forEach(d=>d.classList.remove('open'));
...
/* ---------------- REFRESH & LOGOUT (visual) ---------------- */
document.getElementById('btnRefresh').addEventListener('click', function(){
  this.classList.add('spinning');
  setTimeout(() => { try { renderSection(currentSection); } catch(e){ console.error(e); } this.classList.remove('spinning'); }, 450);
});
...
function renderPrecios(c, themeChanged){
  ...
  if(!themeChanged){
    ...
    document.getElementById('corteChips').addEventListener('click', e => {
      ...
      document.querySelectorAll('#corteChips .chip').forEach(x=>x.classList.remove('active'));
      chip.classList.add('active');
      preciosCorteFiltro = chip.dataset.corte;
  ...
  function fillPreciosTable(){
    const rows = [
      ...p.propio_detalle.filter(x=>x.corte===preciosCorteFiltro).map(x=>({...x, tipo:'Propia'})),
      ...p.competencia_detalle.filter(x=>x.corte===preciosCorteFiltro).map(x=>({...x, tipo:'Competencia'})),
...
/* init */
try {
  renderSection('resumen');
} catch (e) {
  console.error('Error al renderizar el tablero:', e);
}
