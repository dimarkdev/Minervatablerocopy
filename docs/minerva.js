...
function renderResumen(c, themeChanged){
  ...
  if(!themeChanged){
    const ae = r.auditoria_envasados, ap = r.auditoria_pedidos, ac = r.auditoria_carniceria;
    const precioMinRel = ac.por_cadena.reduce((s,x)=>s+x.registros,0);
    c.innerHTML = `
    <div class="kpi-grid">
      ${auditCard('envasados', 'Precios auditados en Envasados', fmtNum(ae.total_pdvs),
        `${ae.por_cadena.length} cadenas — supermercados y Biggie, en el rango elegido`,
        `Puntos de venta distintos (cadena + local) con al menos un precio relevado en Envasados dentro del rango de fechas elegido arriba (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}). Envasados es la única fuente de precios que abarca tanto supermercados como Biggie. Cambiá el rango de fechas para recalcular.`,
        'teal', ae.por_cadena, 'Precios relevados')}
      ${auditCard('pedidos', 'Puntos de venta con pedidos', fmtNum(ap.total_pdvs),
        `${ap.por_cadena.length} cadenas auditadas en el rango elegido`,
        `Puntos de venta distintos que cargaron al menos un pedido dentro del rango de fechas elegido arriba (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}). Hoy Pedidos solo cubre Delimarket, Stock y Superseis — no es el mismo universo que Envasados o Carnicería.`,
        'violet', ap.por_cadena, 'Pedidos cargados')}
      ${auditCard('carniceria', 'Cadenas auditadas en Carnicería', fmtNum(ac.total_cadenas),
        `${fmtNum(precioMinRel)} precios relevados en total, en el rango elegido`,
        `Cadenas con al menos un precio de mostrador relevado en Carnicería dentro del rango de fechas elegido arriba (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}). Biggie no está clausterizado en este canal, por eso no aparece acá.`,
        'amber', ac.por_cadena, 'Precios relevados')}
      ${kpiCard('Cobertura vigente', r.vigentes_30d + ' / ' + ae.total_pdvs,
        `${ae.total_pdvs ? Math.round(100*r.vigentes_30d/ae.total_pdvs) : 0}% del universo de Envasados con precio propio fresco en el rango elegido`,
        `Locales con al menos un precio propio (Pul/Estancia 92) cargado dentro del rango de fechas elegido arriba (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}), sobre el total de PDVs auditados en Envasados en ese mismo rango.`)}
      ${kpiCard('Precio mínimo vs. competencia', r.precio_por_corte.filter(x=>x.diferencial_min_pct<0).length+' / '+r.precio_por_corte.length,
        'cortes donde el precio mínimo de Minerva es más bajo que el de competencia',
        `Compara el precio mínimo relevado de Minerva vs. el precio mínimo relevado de la competencia dentro del rango elegido (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}), corte por corte. Cuenta en cuántos cortes el precio mínimo de Minerva es más barato.`, 'blue')}
      ${kpiCard('Precio máximo vs. competencia', r.precio_por_corte.filter(x=>x.diferencial_max_pct<0).length+' / '+r.precio_por_corte.length,
        'cortes donde el precio máximo de Minerva es más bajo que el de competencia',
        `Compara el precio máximo relevado de Minerva vs. el precio máximo relevado de la competencia dentro del rango elegido (${DATA.meta.ventana.desde} a ${DATA.meta.ventana.hasta}), corte por corte. Cuenta en cuántos cortes el precio máximo de Minerva es más barato.`, 'blue')}
    </div>
    ...
    <div class="card">
      <div class="card-head"><div><div class="card-title">Cortes propios sin competencia relevada</div><div class="card-sub">Combinaciones de Envasados donde hay precio propio pero nunca se relevó un precio de competencia de la misma familia de corte (ej. cualquier Costilla propia compite con cualquier Costilla de competencia, aunque el nombre exacto no coincida) — no depende de la carga de datos de competidores, es un vacío en el relevamiento de precios</div></div></div>
      ${(() => {
        ...
        <div class="alert-note" style="margin-top:8px;">${sinCompetencia.length} combinación(es) corte-marca sin ninguna familia de corte comparable en competencia, dentro del rango de fechas elegido arriba.</div>`;
      })()}
    </div>
    ...
    document.getElementById('covConveniencia').innerHTML = (canalConveniencia && canalConveniencia.total_locales > 0) ? `
      <div class="cov-card-secondary"><div class="cov-top"><div class="cov-cat">Canal Conveniencia</div><div class="cov-pct">${canalConveniencia.pct}%</div></div>
      <div class="progress"><div class="progress-bar pb-grey" style="width:${canalConveniencia.pct}%"></div></div>
      <div class="cov-note">${canalConveniencia.con_propia} de ${canalConveniencia.total_locales} locales con cobertura</div>
