    ...
    charts['crossCompare'] = new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: [...cRows.map(()=>accent), teal], borderRadius: 6, maxBarThickness: 26, minBarLength: 4,
        }]
      },
      options: {
...
function renderPrecios(c, themeChanged){
  ...
  if(!themeChanged){
    ...
    setupPreciosFree(p, textCol, gridBig, teal, accent);
    ...
    drawPrecioFreeChart(teal, accent, textCol, gridBig);
  }
...
function setupPreciosFree(p, textCol, gridBig, teal, accent){
  ...
  initCombo(document.getElementById('precioFreeMinervaInput'), document.getElementById('precioFreeMinervaList'),
    () => freeMinervaList, v => { preciosFreeMinerva = v; renderPrecioFreeResult(teal, accent, textCol, gridBig); });
  ...
  const freeCompChecklist = initChecklistCombo(
    ...
    k => {
      ...
      renderPrecioFreeResult(teal, accent, textCol, gridBig);
    }
  );
...
function renderPrecioFreeResult(teal, accent, textCol, gridBig){
  const p = DATA.precios;
  ...
  drawPrecioFreeChartWith(mRow, cRows, teal, accent, textCol, gridBig);
}

function drawPrecioFreeChartWith(mRow, cRows, teal, accent, textCol, gridBig){
  destroyChart('precioFreeCompare');
...
function drawPrecioFreeChart(teal, accent, textCol, gridBig){
  renderPrecioFreeResult(teal, accent, textCol, gridBig);
}
...
function renderVigencia(c, themeChanged){
  ...
  <div class="kpi-grid">
    ...
    ${expandableCard('vig-vencimientos', 'Vencimientos', fmtNum(v.total),
      ...
      'accent', tablaVencimientos, 'Ver desglose por urgencia')}
  ...
  ${v.criticos.length ? `
  <div class="note-box" style="border-color:var(--accent); background:var(--accent-soft);">
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--accent);"><path d="M12 9v4M12 17h.01M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z"/></svg>
  ...
  <div class="card">
    ...
    ${v.urgentes.length ? `<div class="table-scroll"><table>
      ...
      <tbody>${v.urgentes.map(x=>`<tr${x.dias<=-10?' style="background:var(--accent-soft);"':''}><td>${x.pdv}</td><td>${x.corte}</td><td>${x.marca}</td><td class="num">${fmtNum(x.cantidad)}</td><td class="num">${x.fecha_vence.toLocaleDateString('es-PY')}</td><td class="num">${x.dias<0?`Vencido hace ${Math.abs(x.dias)}d`:(x.dias===0?'Vence hoy':`${x.dias}d`)}</td></tr>`).join('')}</tbody>
