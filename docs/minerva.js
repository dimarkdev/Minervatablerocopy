  ...
  if(!themeChanged){
    ...
    <div class="kpi-grid">
      ...
      ${kpiCard('Presencia Competencia (share)', pr.pct_competencia + '%',
        ...
        'accent')}
      ${kpiCard('Precio mínimo — Minerva vs. Competencia', `${fmtGs(pg.propio_min)} <span style="font-size:14px; color:var(--text3); font-weight:500;">vs</span> ${fmtGs(pg.comp_min)}`,
        ...
        'blue')}
      ${kpiCard('Precio máximo — Minerva vs. Competencia', `${fmtGs(pg.propio_max)} <span style="font-size:14px; color:var(--text3); font-weight:500;">vs</span> ${fmtGs(pg.comp_max)}`,
    ...
    <div class="card">
      <div class="card-head"><div><div class="card-title">Cortes propios sin competencia relevada</div><div class="card-sub">Combinaciones de Envasados donde hay precio propio pero nunca se relevó un precio de competencia de la misma familia de corte, ni tampoco figura un competidor declarado en la hoja de marcas de competencia (ej. cualquier Costilla propia compite con cualquier Costilla de competencia, aunque el nombre exacto no coincida) — en el fondo, todo corte propio compite con alguno, esto es solo un vacío de relevamiento</div></div></div>
      ${(() => {
        // Familia de corte tomada del diccionario armado desde MARCAS-COMPE
        // (COMPETENCIA/FRIGO), no de la primera palabra del nombre -- cubre
        // casos donde el nombre no se parece en nada (ej. "Ojo De Bife"
        // compite con "Bife De Chorizo") y evita mezclar cortes que
        // arrancan igual pero son distintos (ej. "Colita" vs. "Colita De
        // Cuadril"). Si un corte todavía no está cargado en MARCAS-COMPE,
        // cae a la primera palabra como aproximación.
        const alias = DATA.competencia.corte_alias;
        const familiasConCompetenciaRelevada = new Set(DATA.precios.competencia_detalle.map(x => familiaDeCorte(x.corte, alias)));
        const familiasDeclaradas = new Set(DATA.competencia.marcas_compe_por_corte.map(x => familiaDeCorte(x.corte, alias)));
        const sinCompetencia = DATA.precios.propio_detalle
          .filter(x => !familiasConCompetenciaRelevada.has(familiaDeCorte(x.corte, alias)) && !familiasDeclaradas.has(familiaDeCorte(x.corte, alias)))
          .sort((a,b) => a.corte.localeCompare(b.corte) || a.marca.localeCompare(b.marca));
        if (!sinCompetencia.length) return `
        <div class="note-box top"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 11 3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg><span>Todos los cortes propios relevados en esta ventana tienen competencia, ya sea relevada en Envasados o declarada en la hoja de marcas de competencia.</span></div>`;
        return `
        <div class="table-scroll"><table>
          <thead><tr><th>Corte</th><th>Marca propia</th><th class="num">Precio mínimo</th><th class="num">Precio máximo</th><th class="num">Relevamientos</th></tr></thead>
          <tbody>${sinCompetencia.map(x => `
            <tr><td>${x.corte}</td><td>${x.marca}</td><td class="num">${fmtGs(x.precio_min)}</td><td class="num">${fmtGs(x.precio_max)}</td><td class="num">${x.n}</td></tr>`).join('')}
          </tbody></table></div>
        <div class="alert-note" style="margin-top:8px;">${sinCompetencia.length} combinación(es) corte-marca sin competencia relevada NI declarada, dentro del rango de fechas elegido arriba. Revisar si falta agregar el competidor en la hoja de marcas de competencia.</div>`;
      })()}
    </div>
    ...
    const canalConveniencia = r.cobertura_por_categoria_real.find(x => x.categoria === 'Sin clasificar');

    // Composición del universo: cada categoría se muestra como % del total
    // de PDVs auditados en Envasados (no la tasa de cobertura interna de
    // cada categoría por separado) -- entre todas suman 100%, para poder
    // verificar de un vistazo que no falta ni sobra nada.
    const totalUniverso = r.cobertura_por_categoria_real.reduce((s,x) => s + x.total_locales, 0);
    const sumaPct = Math.round(r.cobertura_por_categoria_real.reduce((s,x) => s + x.pct_del_universo, 0) * 10) / 10;
    document.getElementById('covCategoria').innerHTML = categoriasFormales.map(x => {
      ...
      return `<div class="cov-card"><div class="cov-top"><div class="cov-cat">${x.label}${prioBadge}</div><div class="cov-pct">${x.pct_del_universo}%</div></div>
      <div class="progress"><div class="progress-bar pb-green" style="width:${x.pct_del_universo}%"></div></div>
      <div class="cov-note">${x.total_locales} de ${totalUniverso} locales del universo total (${x.con_propia} con Minerva relevada)</div></div>`;
    }).join('') + `<div class="cov-note" style="margin-top:2px; text-align:right;">Suma de todas las categorías (incluyendo Canal Conveniencia): ${sumaPct}%</div>`;

    document.getElementById('covConveniencia').innerHTML = (canalConveniencia && canalConveniencia.total_locales > 0) ? `
      <div class="cov-card-secondary"><div class="cov-top"><div class="cov-cat">Canal Conveniencia</div><div class="cov-pct">${canalConveniencia.pct_del_universo}%</div></div>
      <div class="progress"><div class="progress-bar pb-grey" style="width:${canalConveniencia.pct_del_universo}%"></div></div>
      <div class="cov-note">${canalConveniencia.total_locales} de ${totalUniverso} locales del universo total (${canalConveniencia.con_propia} con cobertura)</div>
      <div class="cov-note" style="margin-top:6px;">Canal de alta presencia territorial, analizado bajo una clasificación propia debido a su formato de atención rápida.</div></div>` : '';
...
function renderPrecios(c, themeChanged){
  ...
  const cortes = [...new Set([...p.competencia_detalle.map(x=>x.corte), ...p.propio_detalle.map(x=>x.corte)])].sort();
