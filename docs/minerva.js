...
function renderVigencia(c, themeChanged){
  ...
  <div class="card">
    ...
    ${v.urgentes.length ? `<div class="table-scroll"><table>
      ...
      <tbody>${v.urgentes.map(x=>`<tr><td>${x.pdv}</td><td>${x.corte}</td><td>${x.marca}</td><td class="num">${fmtNum(x.cantidad)}</td><td class="num">${x.fecha_vence.toLocaleDateString('es-PY')}</td><td class="num">${x.dias<0?`Vencido hace ${Math.abs(x.dias)}d`:`${x.dias}d`}</td></tr>`).join('')}</tbody>
