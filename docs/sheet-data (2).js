...
function extractPriceRows(rows, windowStart, windowEnd){
  ...
  rows.forEach(r => {
    ...
    // por si vuelve a cambiar. Los valores de esta columna hoy ya vienen casi
    // siempre con el prefijo de cadena incluido ("SUPERSEIS - SANBER"), pero
    // se sigue anteponiendo la cadena si todavía no la menciona (ej. Biggie
    // con solo el nombre de la calle, "KUBITSCHEK SEMINARIO").
...
/* Agrupa filas ya filtradas por rango de fechas (inWindow) por cadena,
   contando PDVs únicos (por cadena+local, para no fusionar sucursales de
   distintas cadenas que comparten nombre, ej. "CENTRO") y cantidad de
   registros (relevamientos de precio o pedidos, según la hoja). Se usa
   para las tarjetas expandibles de Resumen (Envasados / Pedidos / Carnicería). */
...
function buildPedidos(rows, windowStart, windowEnd){
  ...
  // Resumen -- por cadena+pdv, ya explicado arriba en auditoriaPorCadena.
  const pdvsUnicosAuditados = new Set(inWin.map(x => x.cadena + '||' + x.pdv)).size;
...
async function fetchDashboardRaw(){
  ...
  // Se piden las 3 hojas obligatorias por separado (no con un solo
  // Promise.all) para poder decir EXACTAMENTE cuál falló si algo sale
  // mal -- un "Failed to fetch" genérico no dice si el problema es el
  // link de "Publicar en la web" vencido, una pestaña renombrada, o un
  // bloqueo de red, y bloquea TODO el tablero por igual sin pista.
  ...
  if (fallidas.length) {
    ...
    throw new Error(`No se pudo leer ${fallidas.length === 1 ? 'la hoja' : 'las hojas'} ${fallidas.map(x=>x.label).join(', ')}. Probá abrir el link directo en una pestaña nueva: si pide iniciar sesión o da error, hay que volver a "Publicar en la web" esa pestaña en Google Sheets. Detalle: ${detalle}`);
  }
...
function buildDashboardData(windowStart, windowEnd){
  if (!_rawCache) throw new Error('Todavía no se cargaron los datos de la planilla.');
  ...
  const pdvsUnicosEnvasados = new Set(priceRowsWindow.map(x => x.cadena + '||' + x.local)).size;
