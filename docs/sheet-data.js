...
/* Foco pedido por Di: Tapa (incl. Tapa De Cuadril), Vacío y Costilla (todas
   sus variantes) son los cortes "premium" de Minerva. Match amplio por
   familia (no nombre exacto) usando el mismo diccionario de MARCAS-COMPE.
   Del lado Minerva: el precio máximo entre esos 3, más el desglose de cada
   uno por separado. Del lado competencia: a pedido de Di, NO se restringe
   a esos 3 cortes -- son los 3 cortes más caros que se auditaron en total. */
/* Sin acentos ni mayúsculas, para comparar por contención de palabra (ej.
   "Costilla Al Plato" empieza con "costilla" aunque tenga tilde en otras
   palabras). Equivalente a normSearch() de minerva.js, pero declarado acá
   también porque sheet-data.js no depende de minerva.js. */
function sinAcentos(s){ return String(s==null?'':s).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase(); }

function esCortePremium(corte, alias){
  const f = sinAcentos(familiaDeCorte(corte, alias));
  return f.startsWith('tapa') || f.startsWith('vacio') || f.startsWith('costilla');
}
function buildCortesPremium(propioDetalle, competenciaDetalle, alias){
  const propioPremium = propioDetalle.filter(x => esCortePremium(x.corte, alias));
  const propioMax = propioPremium.length ? propioPremium.reduce((a,b) => b.precio_max > a.precio_max ? b : a) : null;

  const familias = ['Tapa','Vacío','Costilla'];
  const propioPorFamilia = familias.map(fam => {
    const enFamilia = propioPremium.filter(x => sinAcentos(familiaDeCorte(x.corte, alias)).startsWith(sinAcentos(fam)));
    const max = enFamilia.length ? enFamilia.reduce((a,b) => b.precio_max > a.precio_max ? b : a) : null;
    return { familia: fam, corte: max ? max.corte : null, marca: max ? max.marca : null, precio_max: max ? max.precio_max : null, n: max ? max.n : 0 };
  });

  const competenciaTop3 = [...competenciaDetalle].sort((a,b) => b.precio_max - a.precio_max).slice(0, 3);

  return { propio_max: propioMax, propio_por_familia: propioPorFamilia, competencia_top3: competenciaTop3 };
}
...
async function fetchDashboardRaw(){
  ...
  _rawCache = { pedidosRows, envasadosRows, clasterRows, carniceriaRows, marcasCompeRows, carniceriaError, fetchedAt: new Date() };
  return _rawCache;
}
...
function buildDashboardData(windowStart, windowEnd){
  ...
  const { pedidosRows, envasadosRows, clasterRows, carniceriaRows, marcasCompeRows, carniceriaError, fetchedAt } = _rawCache;
  const now = windowEnd;
  ...
  const cortesPremium = buildCortesPremium(propioDetalle, competenciaDetalle, corteAlias);
