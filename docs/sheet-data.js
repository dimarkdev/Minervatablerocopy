...
function familiaDeCorte(corte, alias){
  ...
  return k.split(' ')[0]; // fallback: corte todavía no cargado en MARCAS-COMPE
}
...
function buildCoberturaCategoria(priceRows, localMap){
  ...
  // Universo total = unión de todos los locales vistos en Envasados, sin
  // importar categoría -- es el 100% contra el que se compone cada
  // categoría (no la cobertura interna de cada una por separado).
  const universoTotal = new Set();
  priceRows.forEach(x => { if (x.local) universoTotal.add(x.local); });
  const rows = [...catAgg.entries()].map(([categoria, g]) => ({
    ...
    pct: pct(g.conPropia.size, g.total.size), // cobertura interna de esa categoría (se sigue calculando, útil aparte)
    pct_del_universo: pct(g.total.size, universoTotal.size), // composición: cuánto pesa esta categoría sobre el 100% del universo
  }));
...
function buildDashboardData(windowStart, windowEnd){
  ...
  return {
    ...
    competencia: {
      ...
      corte_alias: buildCorteAlias(marcasCompeRows),
    },
    cobertura: {
