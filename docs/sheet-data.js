...
function normDisplay(s){
  // Saca guiones sueltos al principio (ej. "- Entre Costilla" -> "Entre
  // Costilla") -- pasa cuando la celda de origen quedó con un guion pegado
  // por un problema de tipeo/formato en la carga, y se ve en cascada en
  // toda tarjeta/lista que muestre ese nombre.
  const t = String(s==null?'':s).trim().replace(/^[-–—]+\s*/, '').replace(/\s+/g,' ');
...
function buildCarniceria(rows){
  ...
  // Día con más ofertas en cantidad (no en tasa %) -- distinto de mejorDia,
  // que mide qué día tiene mayor PROPORCIÓN de ofertas sobre lo relevado.
  const diaConMasOfertas = diasConMuestra.length ? diasConMuestra.reduce((a,b) => b.con_oferta > a.con_oferta ? b : a) : null;

  // Cadena con más ofertas y corte con más ofertas, en toda la ventana --
  // a pedido de Di, reemplaza el foco en ciudades.
  const cadenaOfertaAgg = new Map();
  const corteOfertaAgg = new Map();
  inWin.filter(x => !x.es_nuevo && x.precio_oferta != null && x.precio_oferta < x.precio_regular).forEach(x => {
    cadenaOfertaAgg.set(x.cadena, (cadenaOfertaAgg.get(x.cadena) || 0) + 1);
    corteOfertaAgg.set(x.corte, (corteOfertaAgg.get(x.corte) || 0) + 1);
  });
  const cadenaMasOfertas = [...cadenaOfertaAgg.entries()].map(([cadena,n]) => ({ cadena, n })).sort((a,b) => b.n - a.n)[0] || null;
  const corteMasOferta = [...corteOfertaAgg.entries()].map(([corte,n]) => ({ corte, n })).sort((a,b) => b.n - a.n)[0] || null;

  return {
    total_relevamientos: inWin.length,
    pdvs_relevados: new Set(inWin.map(x=>x.pdv).filter(Boolean)).size,
    cadenas_auditadas: new Set(inWin.map(x=>x.cadena).filter(Boolean)).size,
    ...
    dia_mas_ofertas: diaConMasOfertas,
    cadena_mas_ofertas: cadenaMasOfertas,
    corte_mas_oferta: corteMasOferta,
...
function buildVencimientos(vencidosRows){
  ...
  const clasificados = vencidosRows.map(x => {
    ...
    else if (dias === 0) urgencia = 'Vence hoy';
  ...
  const ordenUrgencia = ['Ya vencido', 'Vence hoy', 'Vence en 7 días', 'Vence en 30 días', 'Más adelante'];
  ...
  // Corte que más se repite entre los urgentes (ya vencido + vence hoy +
  // vence en 7 días) -- para identificar el "problema recurrente", el
  // corte que siempre está en alerta de vencimiento, no el que más
  // unidades tiene.
  const conteoUrgente = new Map();
  clasificados.forEach(x => {
    if (x.urgencia !== 'Ya vencido' && x.urgencia !== 'Vence hoy' && x.urgencia !== 'Vence en 7 días') return;
  ...
  return {
    ...
    urgentes: clasificados.filter(x => x.urgencia === 'Ya vencido' || x.urgencia === 'Vence hoy' || x.urgencia === 'Vence en 7 días')
