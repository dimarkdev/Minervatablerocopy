  ...
  rows.forEach(r => {
    ...
    if (corteNuevoRaw && precioCorteNuevo != null) {
      if (precioCorteNuevo >= OUTLIER_MIN && precioCorteNuevo <= OUTLIER_MAX) {
        out.push({ fecha, inWindow, relevador, pdv, local, ciudad, cadena, corte: normDisplay(corteNuevoRaw), marca_nuevo: normDisplay((r['MARCA CORTE NUEVO']||'').trim()), precio_regular: precioCorteNuevo, precio_oferta: null, es_nuevo: true });
