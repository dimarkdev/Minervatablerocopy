    ...
    if(preciosCorteFiltro.length <= 1){
      ...
      charts['preciosDetalle'] = new Chart(document.getElementById('chPreciosDetalle'), {
        ...
        data: { labels: rows.map(x=>x.corte), datasets: [
          { label:'Minerva (rango)', data: rows.map(x=>[x.propio_min, x.propio_max]), backgroundColor: teal, borderRadius:6, maxBarThickness:34 },
          { label:'Competencia (rango)', data: rows.map(x=>[x.comp_min, x.comp_max]), backgroundColor: accent, borderRadius:6, maxBarThickness:34 },
        ] },
